...
sudo docker run -d -p 8880:3000 grafana/grafana
sleep 5 #Give some time to spin the container and bring service up
echo "Done."
echo ""
#Spin zookeeper container
echo -e "Starting zookeeper container on Local port number $zookeeper_port. Please wait.."\n
sudo docker run -d -p $zookeeper_port:2181 -p 2888:2888 -p 3888:3888 -p 8880:8080 zookeeper
sleep 5
echo "Done"
echo ""
#Spin Kafka container.
echo -e "Starting Kafka container on Local port number $kafka_port. Please wait.." \n
sudo docker run -d -e zookeeper_host=$local_ip -e zookeeper_hostname='myzoo' -e zookeeper_port=$zookeeper_port -e kafka_hostname='mykafka' \
	-e kafka_port=$kafka_port -p $kafka_port:$kafka_port ves-kafka
sleep 7
echo "Done."
echo ""
...
sudo docker run -d -p $kafdrop_port:9000 -e KAFKA_BROKERCONNECT=$local_ip:$kafka_port -e JVM_OPTS="-Xms64M -Xmx128M" obsidiandynamics/kafdrop:latest
sleep 5
echo "Done."
echo ""
...
sudo docker run -d -e ves_influxdb_host=$local_ip -e ves_influxdb_port=$influx_port -e ves_grafana_host=$local_ip -e ves_grafana_port=grafana_port \
	-e ves_host='localhost' -e ves_port=$vel_ves_port -e ves_grafana_auth='admin:admin' -e ves_user='user' -e ves_pass='password' -e ves_path=''\
       	-e ves_topic='events' -e ves_loglevel='DEBUG' -p $vel_ves_port:$vel_ves_port ves-collector
sleep 6
echo "Done."
echo ""
#Spin ves agent container.
echo -e "Starting ves agent container on Local port number $. Please wait" \n
...
echo -e "To access grafana dashboard use http://$local_ip:$grafana_port from web browser."  \n
echo -e "Grafana username/password is admin/admin *** DO NOT CHANGE PASSWORD, CLICK SKIP OPTION" \n
echo ""
echo -e "===================================================================================================================" \n
